	...
	where TState : struct, Enum
{
	...
	public void TransitionTo(TState next)
	{
		if (_world is null)
		{
			throw new InvalidOperationException(
				$"状态 {GetType().Name} 必须先通过 EcsWorld.InsertState 安装，才能执行转换。");
		}
	...
	void IEcsStateBinding.Bind(EcsWorld world)
	{
		...
		{
			// P2-2：OnEnter 失败后解绑（_world 恢复 null），否则重试 InsertState 会因
	...
	/// <summary>进入某状态时调用；首次 InsertState 也会进入初始状态。默认无操作。</summary>
