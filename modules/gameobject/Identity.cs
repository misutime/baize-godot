...
//
// 两层身份，禁止混用：
// - EntityId                运行时身份 = Index + Generation；防删除后 ID 复用（Generation 递增）。
// - AuthoringObjectId       静态作者 ID（.bscene/.bprefab 内稳定，O4 启用）；O1 仅占位。
// RuntimeGameObjectHandle == EntityId（含 Generation 的运行时句柄）。

namespace Baize.GameObject;

/// <summary>
/// 运行时对象身份：Index（槽位）+ Generation（防复用）。
/// 旧 EntityId 永不等于新对象：Destroy 后槽位可复用，但 Generation 递增。
/// </summary>
public readonly record struct EntityId(int Index, uint Generation)
{
	...
	internal EntityId NextGeneration() => new(Index, Generation + 1);
}
