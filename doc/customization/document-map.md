...
| `build-profiles.md`                    | 记录构建 profile、脚本 preset、裁剪参数和验证基线。          | 已能构建后，查参数含义、切换 profile、验证软裁剪影响时读。 |
| `getting-started-windows.md`           | Windows 从 clone 到第一次编译运行的操作步骤。                | 新机器配置、首次编译、排查 Windows 工具链时读。            |
| `getting-started-macos.md`             | macOS 从 clone 到第一次编译运行的操作步骤。                  | 新机器配置、首次编译、排查 macOS 启动方式时读。            |
