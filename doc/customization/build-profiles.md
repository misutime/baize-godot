...
```bash
scons platform=macos dev_build=yes accesskit=no angle=no vulkan=no metal=yes generate_bundle=yes -j8
```
...
- `vulkan=no`：macOS 日常开发先关闭 Vulkan，避免依赖 MoltenVK SDK。
- `metal=yes`：macOS 使用 Metal 渲染后端。
- `generate_bundle=yes`：macOS 构建 `.app`，便于直接启动编辑器。
- `-j8`：使用 8 个并行任务。

macOS 构建完成后，编辑器 app 位于：
