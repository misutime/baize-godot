...
>
> **⚠️ 路线状态（2026-08-05 批注）**：本文为 OSR 路径概念基线（历史）——渲染已演进为**非 OSR 窗口模式**
> （main 合并 `f832eae09b`）：A/B/C 三概念（交付方式/帧源/损伤驱动）与 hold 死锁在窗口模式下不适用
> （OS 窗口消息驱动 resize，结构性消除死锁，见《技术详解-WebDock原生子窗口-非OSR可行性分析与取舍.md》§3.2）；
> fork CEF 方案降级为 OSR 路径兜底。本文结论仅适用于已删除的 OSR 代码。
