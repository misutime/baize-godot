...
>
> **⚠️ 路线状态（2026-08-05 批注）**：本文为 OSR 路径 resize 问题的根因档案（历史）——渲染已演进为
> **非 OSR 窗口模式**（main 合并 `f832eae09b`）：OSR hold 死锁与节流/尾随重发兜底随 OSR 代码一并删除，
> 窗口模式结构性无此问题（OS 窗口消息驱动 resize，见《技术详解-WebDock原生子窗口-非OSR可行性分析与取舍.md》§3.2）；
> 文中根因/修复结论仅适用于已删除的 OSR 代码。
