...
> **2026-08-05 批注**：C++ CefViewCore 路线已裁决并落地合入 main（`f832eae09b`，58 提交）；渲染最终态 =
> **非 OSR 窗口模式**。本文"硬门槛：windowless OSR"为选型期技术基础——OSR 能力已验证可用，但最终渲染形态为
> CEF 原生子窗口（窗口模式），OSR 门槛不再构成现行要求（路线取舍见 `页面渲染选型-OSR与非OSR/`）。
>
