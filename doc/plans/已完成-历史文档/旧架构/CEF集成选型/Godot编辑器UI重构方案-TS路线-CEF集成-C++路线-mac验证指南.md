...
>
> **⚠️ 路线状态（2026-08-05 批注）**：本指南验证的"OSR 浏览器→OnPaint→ImageTexture"链路为 OSR 时代形态，
> 已随渲染演进（非 OSR 窗口模式，main `f832eae09b`）被替代——mac 平台差异/构建/分发结论仍有效，
> 渲染相关 Gate（B2 软件 OSR 等）仅作历史记录。
