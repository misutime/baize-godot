...
> 修订：2026-08-07 **启动期焦点死锁发现与修复**：上游 `--wid` 嵌入 + splash 期点击 owner 抢焦点 → 双方主线程冻结（A/B 证明与己方代码无关）；修复 = 启动期双 no-focus（Electron `setFocusable(false)` + Godot `--embedded-no-focus`）+ React loading 遮罩，`editor.ready` 事件精确解除。详见实施记录 §4.4。
> 修订：2026-08-07 **拖动跟随零延迟定案**：owner 每帧跟随（offset 架构，`viewport.set_viewport_offset`）+ 嵌入模式跳过失焦节流（真根因：失焦 10fps）——用户实测拖动肉眼不可分辨；升级路径 (c) WinEventHook 不再需要。详见实施记录 §4.5。
...
> **定案（2026-08-07）**：采用 fork 放开方案——Electron 保持纯 JS，移动/缩放同步闭环留在 Godot 内；Electron 侧 SetWindowPos addon 弃用。**跟随升级路径已实施（2026-08-07，替代 WinEventHook）**：Godot 每帧 `_update_embedded_follow` 按 owner+offset 重组位置（offset 由 `viewport.set_viewport_offset` 维护、`window_set_position` 刷新）；`set_window_rect` 嵌入模式仅改尺寸；叠加嵌入模式跳过失焦低功耗节流——拖动零延迟（用户实测肉眼不可分辨）。WinEventHook 不再需要。
...
| 拖拽感知滞后（W2 验收不过） | 融合观感受损 | **已解决（2026-08-07）**：owner 每帧跟随（offset 架构）+ 嵌入模式跳过失焦节流（真根因 = 失焦 10fps）——实测拖动零延迟；原定 WinEventHook 升级路径不再需要 |
