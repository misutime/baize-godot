...
- **Object+Components 落地（方案文档在 D:\MisuNotes，仓库内阶段文档）**：`doc/plans/object-components/`——O0 源码依赖地图（Node 派生类/NodePath/SceneTree/PackedScene 依赖量化）与 O1 语义契约（GameObject 内核 12 条实现规范）、社区对象模型对照与借鉴（Unity/Flecs/Bevy/EnTT/Stride/Wave/Defold 对照 + B1-B6 采纳决策）、O2 交接文档（O0-O2 成果 + 下一步 O3 做什么 + 关键概念/坑——接手 O3 的同事先读这个）、O3 可读格式契约草案（GameWorldSnapshot ↔ 文本格式规范，O4 `.bscene/.bprefab` 落地引用；改格式先改它）、O4 `.bscene/.bprefab` 格式与实例化（Uid 稳定身份双轨/prefab/override/loader，O4 阶段权威）、O5 GameWorldHost 与 Server Ports（进程宿主 `Sola3dMainLoop` + Backend 接口集 + Port 三通道 + InputFrame，纯 .NET 零依赖 + Godot 适配壳，社区对照见文档 §6）；方针文档 `D:\MisuNotes\3D游戏开发\Godot_ALL_IN_C#\Godot_Fork_GameObject-Components替换Node_源码级落地方案.md`（阶段 O0-O9，分支 `feature/object-components-engine`）；O2 工程侧说明文档 `test-projects/shooter-object-components/HUMAN_FIRST_AUTHORING.md`（Human-first 读懂 GameObject+Components 设计：概念/六步心智模型/与 ECS 对照/检查清单）
