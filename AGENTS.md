...
- **Object+Components 落地（方案文档在 D:\MisuNotes，仓库内阶段文档）**：`doc/plans/object-components/`——O0 源码依赖地图（Node 派生类/NodePath/SceneTree/PackedScene 依赖量化）与 O1 语义契约（GameObject 内核 12 条实现规范）；方针文档 `D:\MisuNotes\3D游戏开发\Godot_ALL_IN_C#\Godot_Fork_GameObject-Components替换Node_源码级落地方案.md`（阶段 O0-O9，分支 `feature/object-components-engine`）
