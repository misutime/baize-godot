...
- 坐标系、渲染方向、面朝向、相机和输入这类底层契约变更，必须从根本规则出发统一修改，不能用局部反号、投影镜像、剔除反转等方式只把当前现象“补到看起来正确”。用户指出的视觉/操作异常，是用来定位“哪里还没有彻底迁移到新坐标系”的线索；处理时要先追根溯源，找不到根因就继续加日志、加断言、做最小复现场景或扩展测试来追踪，不能用补丁隐藏问题。如果确实需要边界转换，必须明确说明边界两侧仍使用不同协议、为什么暂时不能统一、会影响哪些后续系统，并优先继续追踪能否把边界内外一起改成同一坐标契约。
