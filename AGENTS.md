...
- **Sola3d C# 应用统一入口（2026-08-23 建立；2026-08-24 统一到 src/）**：根目录 Sola3d.slnx 收纳全部 Sola3d.* C# 项目，统一在 `src/` 下三级：`src/libs/`（库：gameobject/mainloop/editor/ecs/ecs-generator/host）、`src/tests/`（测试+玩法验证：core-tests + shooter poc）、`src/apps/`（宿主应用：godot-slice 引擎演示宿主、editor-ui Avalonia 编辑器）——dotnet build Sola3d.slnx 一次性构建 C# 侧；Godot 上游绑定（GodotSharp/GodotTools/Godot.NET.Sdk）不在此（属引擎侧独立 sln）。
...
**强制规则**：
- 测试项目一律放仓库内（`src/tests/`），禁止项目外绝对路径（换机器失效——已踩过 refers/ 外部路径坑）；
...
## 15. 编辑器 UX 规则（2026-08-24 用户裁决）：对齐主流引擎操作

**原则**：所有编辑器（`src/apps/editor-ui`）交互语义**对齐主流引擎（Unity/Godot/UE，以 Unity 为基准习惯）**。
可实现性分步落地，但**设计与实现每一新功能时必须先考虑主流标准操作**，不违背用户既有习惯；已实现项更新 ✅。

| 操作 | 主流标准（基准） | 状态 |
|---|---|---|
| 新建对象 | 选中对象时 → 建为其**子级**；无选中 → 顶层；新建后**自动选中**新对象 | ✅ |
| 删除对象 | 删除选中对象（含子树，可 undo）；键盘 Delete | 待实现 |
| 撤销 / 重做 | Ctrl+Z / Ctrl+Shift+Z（或 Ctrl+Y） | 待实现（undo 核心已就绪，接快捷键） |
| 保存 / 加载 | Ctrl+S 保存 / Ctrl+O 打开；工具栏 + 主菜单 | 待实现（对话框已就绪，接快捷键） |
| 层级 | 拖放改变父子（父=放置目标），选中保持；展开状态持久 | 待实现（增量树就绪） |
| Inspector | 编辑即改 + undo；Esc 还原输入 | 部分（编辑即改 ✅） |
| 重命名 | F2 重命名选中对象 | 待实现 |
| 复制/粘贴 | Ctrl+C/V 复制对象（含组件/属性，新 Uid） | 待实现 |
| 选择 | 点选；Esc 取消选择；多选（后续） | 部分（点选 ✅） |
| 3D 预览 | 中间面板“打开/刷新 3D 预览”→ 跨进程 godot-slice `--editor-preview` 真渲染；保存后 mtime 自动重投影 | ✅（跨进程 MVP；同进程嵌入=M2 Native 壳） |

**落地**：逐项实现并在上表更新 ✅；不得引入与上表冲突的交互。
...
- **统一心智模型（2026-08-23 采纳）**：**Design World（设计世界）↔ Runtime World（运行世界）**——Design 回答"世界应该是什么样"（`.bscene`/`.bprefab`/`Uid`/Design document model），Runtime 回答"现在正在发生什么"（`GameWorld`/`ObjectId`/组件实例）。完整对照表见落地方案 §1.2a 与 O2 交接文档 §A；`Authoring` 一词已弃用（注：**"Human-first Authoring" 是独立方法论专名**——先读"世界里有什么/世界怎么步进"再读底层机制的代码哲学，与 Design World 无关，保留使用；同名文档 `HUMAN_FIRST_AUTHORING.md` 不复名）。
- **Object+Components 落地（方案文档在 D:\MisuNotes，仓库内阶段文档）**：`doc/plans/object-components/`——O0 源码依赖地图（Node 派生类/NodePath/SceneTree/PackedScene 依赖量化）与 O1 语义契约（GameObject 内核 12 条实现规范）、社区对象模型对照与借鉴（Unity/Flecs/Bevy/EnTT/Stride/Wave/Defold 对照 + B1-B6 采纳决策）、O2 交接文档（O0-O2 成果 + 下一步 O3 做什么 + 关键概念/坑——接手 O3 的同事先读这个）、O3 可读格式契约草案（GameWorldSnapshot ↔ 文本格式规范，O4 `.bscene/.bprefab` 落地引用；改格式先改它）、O4 `.bscene/.bprefab` 格式与实例化（Uid 稳定身份双轨/prefab/override/loader，O4 阶段权威）、O5 GameWorldHost 与 Server Ports（进程宿主 `Sola3dMainLoop` + Gateway 接口集 + Port 三通道 + InputFrame，纯 .NET 零依赖 + Godot 适配壳，社区对照见文档 §6）、O6 gateway 与最小 vertical slice（Transform/Mesh/StaticCollider 数据组件 + SceneProjector 投影 + GodotRenderingServer 真桥——状态 C 第一块砖，序列化白名单扩 Vector3/Quaternion R27）、O7 编辑器第一切片（`Sola3d.Editor` 纯 .NET 编辑器核心：EditorSession Design 文档编辑 + Undo 栈 + .bscene 保存/重载闭环；DesignPreviewHost 预演世界投影 + godot-slice GodotDesignPreview 桥——3D view 显示路径就位）；**O8 交接文档（后续开发接手者先读）**：O8-交接文档.md（O3–O7.5 成果全景/统一入口 Sola3d.slnx/测试基线/下一步编辑器壳+域迁移+进程入口收尾/已知坑）. **团队概念文档（先读再动手）**：`术语表-双世界与内核边界.md`（术语权威：GameWorld 属 Runtime World 侧不加前缀 / Gateway 对接 Godot Core 经 Server API）、`Node能力去向与复用边界.md`（Node 能力分三类：机器能力复用 Server / 组织能力已重写 / 编辑器壳 O7，#不是每个 Node 都有组件）、`性能路径与跨语言边界.md`（GameWorld 纯 C# 零跨语言、GDScript 调节点优势无对应场景、未来薄绑定通道加速 gateway）、`Gateway对接地图与迁移路线.md`（后续迁移参考：Godot 内核待对接 Gateway 全集/实现状态/渲染与物理两个已立样板模式/优先级路线/实测坑——做新域迁移先读）；方针文档 `D:\MisuNotes\3D游戏开发\Godot_ALL_IN_C#\Godot_Fork_GameObject-Components替换Node_源码级落地方案.md`（阶段 O0-O9，分支 `feature/object-components-engine`）；O2 工程侧说明文档 `src/tests/shooter-object-components/HUMAN_FIRST_AUTHORING.md`（Human-first 读懂 GameObject+Components 设计：概念/六步心智模型/与 ECS 对照/检查清单）
