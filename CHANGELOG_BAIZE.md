...
- **Object+Components 落地：O2 作者体验优化**（`test-projects/shooter-object-components/`）：①工厂去样板——`ShooterFactory` 由「先 AddComponent<T>() 再 GetComponent 赋属性」改为 `AddComponent(new X { ... })` 一次性初始化（复用已有 `AddComponent<T>(T)` 重载），每处少一半 GetComponent 查询、初始值一行可见；②`ShooterGame.Step` 规划顺序用 `PlanPhase` 枚举显式化——新增 `ShooterWorld.PlanPhase`（PlayerInput→Enemy→Projectile）与 `PlanMotion(world, delta, tickIndex, phase)` 编排方法，`Step` 按 `static readonly PlanPhase[]` 遍历，替代原先三个裸 foreach，顺序权威集中一处、可读性提升，且不引入全局排程器/框架。纯重构、行为等价（组件组合/初始化值/规划顺序逐一一致），独立 review APPROVE；O2 库 0 错误、poc 15 项全绿、O1 内核 147/0 不回归。HUMAN_FIRST_AUTHORING 已同步描述。
