...
## 2026-08-22
- **Object+Components 落地：全局服务改名为资源（Service→Resource，对齐 Bevy）**（`modules/gameobject/` + O2）：`GameWorld.AddService`→`AddResource`、`GetService`→`GetResource`、`HasService`→`HasResource`、`_services`→`_resources`、端口注释 `// Services`→`// Resources`——全局单例概念对齐 Bevy `Resource`（`Res<T>`；Bevy 把配置/状态/计时器等全局单例统一叫 Resource）。`ShooterServices.cs`→`ShooterResources.cs`（文件头/注释同步）。`InputService`/`MatchController`/`SpawnConfig`/`SpawnState` 等具体资源名保留（Bevy 里具体资源也各有名，如 `Input<...>`，不强制都带 Resource 后缀）。影响 O1 契约 §11（Services 端口）与 O2 装配。纯改名不改行为，O1 内核 147/0、poc 15 项全绿；HUMAN_FIRST_AUTHORING 已同步。
- **Object+Components 落地：O2 `ShooterWorld`→`ShooterWorldHelper`**（`test-projects/shooter-object-components/`）：改名静态工具类——它实为「世界的查询/编排工具」（`AllObjects`/`QueryObjects`/`CanTick`/`PlanMotion` + `PlanPhase` 枚举），非"世界本身"，改名 `ShooterWorldHelper` 更贴切；`ShooterWorld.cs`→`ShooterWorldHelper.cs`。`ShooterGame`（主控入口 CreateWorld/Install/RunFrame/Restart）保持不变。纯改名不改行为，poc 15 项全绿、O1 内核 147/0 不回归；HUMAN_FIRST_AUTHORING 已同步。
- **Object+Components 落地：O2 行为组件命名 Behavior→Action**（`test-projects/shooter-object-components/`）：应反馈改术语——`MoveBehavior`→`MoveAction`、`PlayerControllerBehavior`→`PlayerControllerAction`、`WeaponBehavior`→`WeaponAction`、`BulletBehavior`→`BulletAction`、`EnemyControllerBehavior`→`EnemyControllerAction`、`EnemySpawnerBehavior`→`EnemySpawnerAction`；`ShooterBehaviors.cs`→`ShooterActions.cs`。理由：`Behavior` 偏抽象且易与 Unity `MonoBehaviour` 混淆；改 `Action`（对齐 Svelte `use:action`，自带生命周期、更直白）。纯改名不改行为，poc 15 项全绿、O1 内核 147/0 不回归；HUMAN_FIRST_AUTHORING 与 CHANGELOG 历史条目中的旧类名保留（历史如实，此处新条目说明改名事实）。
- **Object+Components 落地：O2 `ShooterGame.Step`→`RunFrame`**（`test-projects/shooter-object-components/`）：改名推进入口——`Step`（步进）隐晦、易与"步进调试/单步计数"混淆，且与 `world.Tick` 语义重叠；改 `RunFrame`（跑一帧）直白点明"推进一个逻辑帧"。`ShooterGame.RunFrame(world)` 内部仍为「控制器先 `PlanMotion` 提交本帧计划 → `world.Tick` 统一执行移动+碰撞」。纯改名不改行为，poc 15 项全绿、O1 内核 147/0 不回归；HUMAN_FIRST_AUTHORING 已同步。（注：`modules/ecs/` 的 `Step(InputFrame)` 是另一概念，非本改名对象。）
